...
class FeatureExtractor:
    """
    Feature extraction for bus arrival prediction.
    
    This class provides comprehensive feature engineering capabilities
    for bus arrival prediction including temporal, spatial, and route-specific
    features optimized for machine learning models.
    ...
    def extract_temporal_features(self, live_data: pd.DataFrame) -> pd.DataFrame:
        ...
        encodings for better model performance.
        ...
        logger.info(f"Added {len([col for col in live_data.columns if col.startswith(('hour', 'day', 'month', 'week', 'is_'))])} temporal features")
        return live_data
